...
namespace iTuner
{
	...
	internal static class NetworkStatus
	{
		...
		private static IPInterfaceProperties properties;
		...
		/// <summary>
		/// Determines the domain name of the best possible active network adapter on the local
		/// machine.  The best adapter is first one that is both active and has both receive and
		/// send counts greater than zero.
		/// </summary>
		/// <returns>
		/// The domain name or DNS suffix of the local host.
		/// </returns>

		public static string GetBestDomainName ()
		{
			if (isAvailable && (properties != null))
			{
				...
				foreach (NetworkInterface face in interfaces)
				{
					...
					if (face.OperationalStatus == OperationalStatus.Up)
					{
							...
							(face.NetworkInterfaceType != NetworkInterfaceType.Loopback))
						{
							...
							{
								properties = face.GetIPProperties();
								return true;
							}
						}
					}
